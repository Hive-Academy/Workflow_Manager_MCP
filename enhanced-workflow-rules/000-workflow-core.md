...
- Input **1** → Proceed with direct assistance
- Input **2** → Load boomerang role rules, initialize structured workflow
- No clear input → Re-present choice with clarification
...
□ MCP Context: Current task and workflow state available
...
- **Continue**: Rules loaded + MCP context available → Proceed with role work
...
- **Get Context**: Need current context → Use MCP query tools
...
```
MCP CONTEXT CHECK:
□ Task Context: Retrieved via query_task_context successfully
□ Implementation Evidence: Available in MCP storage
□ Quality Validation: MCP evidence and metrics accessible
□ Strategic Context: Delegation history and transitions available
...
workflow_operations({
  ...
  escalationData: {
    reason:
      'architecture_violation|integration_conflict|performance_issue|security_concern|scope_expansion',
...
□ Current state verification with functional testing evidence stored in MCP
...
□ Implementation plan created with logical 3-8 subtask batches stored in MCP
□ Technical decisions documented following established patterns in MCP analysis
□ Quality standards understood and applicable to current task stored in context
□ Strategic guidance provided with specific patterns and quality constraints

**Before Final Completion:**
□ All acceptance criteria verified with specific evidence stored in MCP
□ Code review approval with comprehensive testing validation stored in MCP
□ System integration validated through testing workflows with MCP tracking
□ User delivery prepared with complete documentation and actionable next steps
□ Strategic decisions and quality improvements documented in MCP
