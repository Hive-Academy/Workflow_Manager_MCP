...
- ❌ Routine delegation messages (use efficient delegation format instead)
- ❌ Progress updates during normal workflow
- ❌ Status confirmations that duplicate MCP data
- ❌ General observations about implementation approach
...
1. Can the next role get this info via MCP? → NO NOTE NEEDED
2. Is this routine workflow communication? → NO NOTE NEEDED
3. Does this require immediate user action/decision? → CONSIDER NOTE
4. Is this critical context missing from MCP data? → CONSIDER NOTE
```
...
✅ Research necessity properly evaluated with clear framework
✅ Memory bank analysis completed with specific findings
✅ Source code analysis documented with relevant patterns
✅ Delegation message is token-efficient and focused
```
...
✅ Batch integration verified and system coherence confirmed
✅ Completion report created with comprehensive documentation
✅ User delivery prepared with concise summary and MCP reference
```
...
✅ Information cannot be obtained via MCP by next role
✅ Content is under 50 words and action-oriented
✅ Note provides specific next steps or decisions needed
```
