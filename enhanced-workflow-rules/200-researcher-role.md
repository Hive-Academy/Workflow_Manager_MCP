...
### Step 1: Task Context Retrieval (1 MCP call - if context not fresh)
...
### Step 6: Comprehensive Research Report Creation (1 MCP call)
...
mutate_data({
  ...
  data: {
    ...
    methodology: 'Research approaches and sources used',
    findings:
      'Detailed findings organized by research question with supporting evidence',
    recommendations:
      'Specific, actionable implementation guidance prioritized by importance',
    riskAssessment:
      'Identified risks, mitigation strategies, and alternative approaches',
    references: [
      'Authoritative source 1 with URL and relevance',
      'Technical documentation 2 with specific sections',
      'Code example 3 with implementation patterns',
    ],
    nextSteps:
      'Recommended next steps for architecture and implementation phases',
  },
});
```

### Step 7: Return to Boomerang with Findings (1 MCP call)
...
workflow_operations({
  ...
  toRole: 'boomerang',
  message:
    'Research complete. Key findings: [2-3 sentence summary of most critical discoveries and recommendations].',
});
```
