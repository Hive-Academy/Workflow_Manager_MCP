...
**🔄 Switching to Senior Developer mode** when:
...
├── ST-001: Database schema setup
├── ST-002: User authentication service
├── ST-003: Core API endpoints
└── ST-004: Input validation middleware
...
4. **Update implementation plan document with technical details**:
   - **Mark subtask status**: not-started → in-progress → completed with evidence
...
5. **Make logical, well-documented commits**:
   - **Commit related changes together** with clear scope and purpose
   - **Use descriptive messages**: "feat(auth): implement JWT validation service with error handling - ST-002"
   - **Maintain clean, reviewable commit history** for effective code review
   - **Include testing and quality verification** in commit scope
```
...
├── ST-005: Login form component (independent)
├── ST-006: User profile component (independent)
├── ST-007: Navigation component (independent)
├── ST-008: Form validation utilities (independent)
└── ST-009: Component integration (depends on ST-005-008)
...
├── ST-010: Backend API integration (foundation)
├── ST-011: Frontend-backend data flow (depends on ST-010)
├── ST-012: User workflow testing (depends on ST-011)
└── ST-013: Performance optimization (depends on ST-012)
...
├── Security fix in authentication (from B001)
├── Performance improvement in API endpoints (from B001)
├── UI consistency fix in login form (from B002)
└── Integration test enhancement (from B003)
...
```
✅ EFFICIENT: "Starting batch B002 for TSK-007. Frontend components batch with 5 related subtasks. Implementation approach planned."
...
```
✅ EFFICIENT: Focus on batch completion, not individual subtask updates
❌ AVOID: "ST-005 is 50% complete, working on form validation logic..."
❌ AVOID: "ST-006 encountered minor issue with state management..."
❌ AVOID: "ST-007 testing in progress, found one edge case..."
```

**Completion Reporting:**

```
✅ EFFICIENT: "Batch B002 complete for TSK-007. All 5 frontend components implemented and integrated. User workflows tested end-to-end. Ready for B003."

❌ VERBOSE: Detailed description of every subtask implementation
```
...
```
❌ DON'T ESCALATE: Individual subtask challenges that can be resolved within batch
✅ ESCALATE: Architecture conflicts affecting multiple subtasks
✅ ESCALATE: Missing dependencies that block entire batch
✅ ESCALATE: Technical decisions needed that affect batch design
```
