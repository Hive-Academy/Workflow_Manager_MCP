...
export const GetTaskContextSchema = z.object({
  ...
  taskName: z.string().optional().describe('Optional task name for confirmation/disambiguation'),
  sliceType: z.enum([
  ...
  ]).optional().default('FULL').describe('Type of context slice to retrieve'),
  includeRelated: z.boolean().optional().default(true).describe('Whether to include related data (subtasks, comments, etc.)'),
  maxComments: z.number().int().min(1).max(50).optional().default(10).describe('Maximum number of recent comments to include'),
  maxDelegations: z.number().int().min(1).max(20).optional().default(5).describe('Maximum number of recent delegations to include'),
});
...
export const TaskContextResponseSchema = z.object({
  ...
  delegationHistory: z.array(z.object({
    id: z.number().int(),
    fromMode: z.string(),
    toMode: z.string(),
  ...
  researchReports: z.array(z.object({
    id: z.number().int(),
    title: z.string(),
    summary: z.string(),
    createdAt: z.date(),
