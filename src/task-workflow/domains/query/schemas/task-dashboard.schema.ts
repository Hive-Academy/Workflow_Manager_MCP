...
export const TaskDashboardSchema = z.object({
  ...
  includeArchived: z
    ...
    .optional()
    .default(false)
    .describe('Whether to include archived tasks'),
  timeframe: z
    .enum(['today', 'week', 'month', 'all'])
    .optional()
    .default('all')
    .describe('Time frame for dashboard data'),
});

export type TaskDashboardParams = z.infer<typeof TaskDashboardSchema>;

// Add the expected schema alias for service compatibility
export const TaskDashboardParamsSchema = TaskDashboardSchema;
...
export const TaskDashboardResponseSchema = z.object({
  ...
  tasksByStatus: z
    .record(z.number().int())
    .describe('Count of tasks by status'),
  tasksByMode: z
    .record(z.number().int())
    .describe('Count of tasks by current mode'),
  tasksByPriority: z
    .record(z.number().int())
    .describe('Count of tasks by priority'),

  // Workflow metrics
  averageTaskDuration: z
    .number()
    .optional()
    .describe('Average task completion time in hours'),
  averageRedelegations: z
    .number()
    .optional()
    .describe('Average number of redelegations per task'),
  workflowEfficiency: z
    .number()
    .optional()
    .describe('Overall workflow efficiency score (0-1)'),
