...
export const WorkflowMapSchema = z.object({
  taskId: z
    .string()
    .optional()
    .describe('Optional Task ID to highlight its current mode in the map'),
  showMetrics: z
    .boolean()
    .optional()
    .default(true)
    .describe('Whether to include workflow metrics'),
  includeTransitionHistory: z
    .boolean()
    .optional()
    .default(false)
...
export const WorkflowMapResponseSchema = z.object({
  // Workflow structure
  workflowNodes: z.array(
    z.object({
      id: z.string().describe('Node ID (mode name)'),
      name: z.string().describe('Display name'),
      type: z
        .enum(['role', 'stage', 'decision'])
        .describe('Type of workflow node'),
      ...
      currentTasks: z
        .array(z.string())
        .describe('Tasks currently in this mode'),
      ...
      averageTime: z
        .number()
        .optional()
        .describe('Average time spent in this mode (hours)'),
  ...
  workflowEdges: z.array(
    z.object({
      from: z.string().describe('Source mode'),
      to: z.string().describe('Target mode'),
      type: z
        .enum(['normal', 'delegation', 'redelegation', 'escalation'])
      ...
      transitionCount: z
        .number()
        .int()
        .describe('Number of times this transition occurred'),
      averageTransitionTime: z
        .number()
        .optional()
        .describe('Average time for this transition (minutes)'),
      successRate: z
        .number()
        .optional()
        .describe('Success rate for this transition (0-1)'),
    }),
  ),
  ...
  highlightedTask: z
    .object({
      ...
      stageProgress: z
        .number()
        .describe('Progress through current stage (0-1)'),
  ...
  workflowHealth: z.object({
    ...
    recommendedOptimizations: z
      .array(z.string())
      .describe('Suggested workflow improvements'),

    // Performance indicators
    averageTaskCycleTime: z
      .number()
      .describe('Average time from start to completion (hours)'),
