...
async function testE2EReportGeneration() {
  ...
  try {
    ...
    sampleTasks.forEach((task, idx) => {
      console.log(`   ${idx + 1}. ${task.taskId}: "${task.name}" (${task.status}, ${task.priority})`);
    });

    if (taskCount === 0) {
      throw new Error('No tasks found in database. Please create some test tasks first.');
    }
    ...
    console.log(`   Content matches generated: ${fileContent === htmlContent ? 'YES' : 'NO'}`);
    ...
    const chartIDTests = [
      {
        ...
        test: () => {
          const match = htmlContent.match(/<canvas id="([^"]*)"[^>]*>.*?Status.*?Distribution/s);
      ...
      },
      {
        ...
        test: () => {
          const match = htmlContent.match(/<canvas id="([^"]*)"[^>]*>.*?Priority.*?Distribution/s);
          return match && match[1] && match[1] !== 'undefined';
        }
      },
      {
        ...
        test: () => {
          const match = htmlContent.match(/<canvas id="([^"]*)"[^>]*>.*?Completion.*?Trend/s);
          return match && match[1] && match[1] !== 'undefined';
        }
      },
      {
        ...
        test: () => {
          const match = htmlContent.match(/<canvas id="([^"]*)"[^>]*>.*?Role.*?Performance/s);
          return match && match[1] && match[1] !== 'undefined';
        }
      },
      {
    ...
    const cardDesignTests = [
      {
        ...
        test: () => htmlContent.includes('task-card') || htmlContent.includes('card')
      },
      {
        name: 'Subtasks section present',
        test: () => htmlContent.includes('subtask') || htmlContent.includes('Subtasks')
      },
      {
        name: 'Progress bars for tasks',
        test: () => htmlContent.includes('progress') || htmlContent.includes('Progress')
      },
      {
        name: 'Modern card styling',
        test: () => htmlContent.includes('rounded-') && htmlContent.includes('shadow-')
      },
      {
        name: 'Task details focus',
        test: () => htmlContent.includes('Task Details') || htmlContent.includes('task detail')
      },
      {
        name: 'Reduced delegation emphasis',
        test: () => {
          const delegationCount = (htmlContent.match(/delegation/gi) || []).length;
    ...
    const dataIntegrationTests = [
      {
        ...
        test: () => sampleTasks.some(task => htmlContent.includes(task.taskId) || htmlContent.includes(task.name))
      },
      {
        name: 'Task status distribution',
        test: () => htmlContent.includes('Status Distribution') || htmlContent.includes('status')
      },
      {
        name: 'Priority analysis',
        test: () => htmlContent.includes('Priority') || htmlContent.includes('priority')
      },
      {
        name: 'Performance metrics',
        test: () => htmlContent.includes('Performance') || htmlContent.includes('metrics')
      },
      {
        name: 'Interactive functionality',
        test: () => htmlContent.includes('Chart(') && htmlContent.includes('addEventListener')
      }
    ];
    ...
    const basicValidations = [
      {
        ...
        test: () => htmlContent.includes('<!DOCTYPE html>') && htmlContent.includes('<html') && htmlContent.includes('</html>')
      },
      {
      ...
      },
      {
        ...
        test: () => htmlContent.includes('tailwind') || htmlContent.includes('bg-') || htmlContent.includes('text-')
      },
      {
        name: 'JavaScript functionality',
        test: () => htmlContent.includes('<script>') && htmlContent.includes('Chart(')
      },
      {
        name: 'Responsive design',
        test: () => htmlContent.includes('responsive') || htmlContent.includes('md:') || htmlContent.includes('lg:')
      }
    ];
    ...
    console.log('==================================================');
    console.log(`🎯 Chart ID Fix: ${passedChartIDTests}/${chartIDTests.length} tests passed`);
    console.log(`💳 Card Design: ${passedCardTests}/${cardDesignTests.length} tests passed`);
    console.log(`🔗 Data Integration: ${passedDataTests}/${dataIntegrationTests.length} tests passed`);
    console.log(`🔍 Basic Structure: ${passedBasicTests}/${basicValidations.length} tests passed`);
    console.log(`⚡ Performance: ${passedPerformanceTests}/${performanceTests.length} tests passed`);
    
    const totalTests = chartIDTests.length + cardDesignTests.length + dataIntegrationTests.length + basicValidations.length + performanceTests.length;
    const totalPassed = passedChartIDTests + passedCardTests + passedDataTests + passedBasicTests + passedPerformanceTests;
    const successRate = ((totalPassed / totalTests) * 100).toFixed(1);
    
    console.log(`\n🏆 OVERALL SUCCESS RATE: ${totalPassed}/${totalTests} (${successRate}%)`);

    if (passedChartIDTests === chartIDTests.length) {
      console.log('✅ PRIMARY OBJECTIVE ACHIEVED: Chart ID undefined issue is FIXED!');
    } else {
      console.log('❌ PRIMARY OBJECTIVE NOT FULLY ACHIEVED: Chart ID issues still exist');
    }

    if (passedCardTests >= 4) {
      console.log('✅ SECONDARY OBJECTIVE ACHIEVED: Card-based design successfully implemented!');
    } else {
      console.log('❌ SECONDARY OBJECTIVE NEEDS WORK: Card-based design needs improvement');
    }

    if (passedPerformanceTests >= 3) {
      console.log('✅ PERFORMANCE OBJECTIVE ACHIEVED: Report generation is fast and efficient!');
    } else {
      console.log('❌ PERFORMANCE OBJECTIVE NEEDS WORK: Report generation performance needs improvement');
    }

    console.log(`\n📁 Generated report available at: ${savedFilePath}`);
    console.log(`📈 Performance: ${totalProcessingTime}ms total processing time`);
    console.log(`💾 File size: ${fileSizeKB} KB`);
    console.log(`📊 Data coverage: ${taskMatches} task references, ${delegationMatches} delegation references`);
