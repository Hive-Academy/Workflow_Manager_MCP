...
async function testTaskDetailWithDelegations() {
  ...
  try {
    ...
    console.log(
      `   - Delegation records: ${taskData.delegationHistory?.length || 0}`,
    );
    console.log(
      `   - Codebase analysis: ${taskData.codebaseAnalysis ? 'Present' : 'Not available'}\n`,
    );

    if (
      !taskData.delegationHistory ||
      taskData.delegationHistory.length === 0
    ) {
      console.log(
        '❌ This task has no delegation records. Let me find one that does...\n',
      );
      ...
      if (tasksWithDelegations.length > 0) {
        ...
        console.log(
          `🔄 Found task with delegations: ${taskWithDelegations.taskId}`,
        );
        console.log(`   - Name: ${taskWithDelegations.name}`);
        console.log(
          `   - Delegation records: ${taskWithDelegations.delegationRecords.length}\n`,
        );

        // Test with this task instead
        const delegationTaskData = await taskDetailService.generateReport(
          taskWithDelegations.taskId,
        );

        console.log(
          '📄 Generating task detail HTML report with delegation history...',
        );
        const htmlGenerationStart = Date.now();
        const htmlContent =
          generatorService.generateTaskDetail(delegationTaskData);
        ...
        const delegationMatches = (htmlContent.match(/delegation/gi) || [])
          .length;
        const workflowMatches = (htmlContent.match(/workflow/gi) || []).length;
        const transitionMatches = (htmlContent.match(/transition/gi) || [])
          .length;
        ...
        const hasFromToRoles =
          htmlContent.includes('fromMode') || htmlContent.includes('toMode');
        const hasDelegationTimestamps = htmlContent.includes(
          'delegationTimestamp',
        );
        ...
        const hasKeyInsights =
          htmlContent.includes('Key Insights Summary') ||
          htmlContent.includes('grid grid-cols-1 md:grid-cols-3');
        const hasArchitectureSection = htmlContent.includes(
          'Architecture & Technology',
        );
        const hasProblemsSection = htmlContent.includes('Problems Identified');
        const hasImplementationSection = htmlContent.includes(
        ...
        console.log(
          `   ✅ Key insights summary: ${hasKeyInsights ? 'PRESENT' : 'MISSING'}`,
        );
        console.log(
          `   ✅ Architecture section: ${hasArchitectureSection ? 'PRESENT' : 'MISSING'}`,
        );
        console.log(
          `   ✅ Problems section: ${hasProblemsSection ? 'PRESENT' : 'MISSING'}`,
        );
        console.log(
          `   ✅ Implementation section: ${hasImplementationSection ? 'PRESENT' : 'MISSING'}`,
        );
        console.log(
          `   ✅ Quality section: ${hasQualitySection ? 'PRESENT' : 'MISSING'}`,
        );
        console.log(
          `   ✅ Enhanced gradient cards: ${hasGradientCards ? 'PRESENT' : 'MISSING'}\n`,
        );
        ...
        console.log(
          `   📊 Delegation records: ${delegationTaskData.delegationHistory?.length || 0}`,
        );
        console.log(
          `   🎨 Enhanced UX: Modern card-based design with improved visual hierarchy\n`,
        );
        ...
        console.log(
          '🎉 Task Detail Report with Delegations Test Completed Successfully!',
        );
        console.log('   ✅ Delegation history functionality working');
        console.log(
          '   ✅ Enhanced codebase analysis UX architecture implemented',
        );
        console.log(
          '   ✅ Modern visual design with improved information hierarchy',
        );
      } else {
