...
var TASK_TRACKING_PATH = path.join(REPO_ROOT, "task-tracking");
function getEmojiForMode(mode) {
  switch (mode) {
    ...
    case "architect-mode":
      return "\u{1F3DB}\uFE0F";
    case "senior-developer-mode":
      return "\u{1F468}\u200D\u{1F4BB}";
    case "code-review-mode":
      return "\u{1F50D}";
    case "researcher-expert-mode":
      return "\u{1F52C}";
    default:
      return "\u2753";
  }
...
async function getTaskPath(identifierOrBaseId, taskName) {
...
async function getTaskState(taskPath) {
  try {
    ...
    return {
      ...
      progress: parsedState.progress !== void 0 ? parsedState.progress : 0,
      ...
      createdAt: parsedState.createdAt || (/* @__PURE__ */ new Date()).toISOString(),
      updatedAt: parsedState.updatedAt || (/* @__PURE__ */ new Date()).toISOString(),
      ...parsedState
      // Spread the rest of the parsed state
    };
  } catch (err) {
...
function registerDelegateTaskTool(server, registry) {
  ...
  const handler = async (args, extra) => {
    ...
    try {
      ...
      taskState.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
      ...
      const textContent = `# \u{1F504} Task Delegated Successfully \u{1F504}
...
${message || `Task delegated from ${fromMode} to ${toMode}`}`;
      ...
      return { content: [{ type: "text", text: `Failed to delegate task: ${err.message}` }], isError: true };
    }
  };
  server.tool(toolName, description, schemaRawShape, handler);
  registry.set(toolName, handler);
}
function registerCompleteTaskTool(server, registry) {
  ...
  const handler = async (args, extra) => {
    ...
    try {
      ...
      if (taskState.currentOwner && taskState.currentOwner !== mode) {
        return { content: [{ type: "text", text: `Failed to complete task: Task is currently owned by ${taskState.currentOwner}, not ${mode}.` }], isError: true };
      }
      let delegator = null;
      ...
      taskState.progress = status === "completed" ? 1 : taskState.progress;
      taskState.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
      ...
      await fs.writeFile(statePath, JSON.stringify(taskState, null, 2), "utf8");
      const displayTaskName = taskState.taskName || path.basename(effectiveTaskPath);
      const textContent = `# \u2705 Task ${status.toUpperCase()} \u2705
...
## Next Steps
${delegator ? `1. Complete any remaining work in the current ${mode.replace("-mode", "")} mode
...
${notes || `Task ${status} by ${mode}`}`;
      ...
      return { content: [{ type: "text", text: `Failed to complete task: ${err.message}` }], isError: true };
    }
  };
  server.tool(toolName, description, schemaRawShape, handler);
  registry.set(toolName, handler);
}
function registerGetCurrentModeForTaskTool(server, registry) {
  ...
  const handler = async (args, extra) => {
    ...
    try {
      ...
      return {
        content: [{ type: "text", text: `Current mode for task '${displayTaskName}' (ID: ${taskState.taskId}): ${currentMode}` }],
        currentMode
      };
    } catch (err) {
      if (extra && extra.logger) extra.logger.error(`GetCurrentMode Error: ${err.message}`, err);
      return { content: [{ type: "text", text: `Failed to get current mode: ${err.message}` }], isError: true };
    }
  };
  server.tool(toolName, description, schemaRawShape, handler);
  registry.set(toolName, handler);
}
function registerContinueTaskTool(server, registry) {
  ...
  const handler = async (args, extra) => {
    ...
    try {
      ...
      const displayTaskName = taskState.taskName || taskId;
      const textContent = `# \u25B6\uFE0F Continuing Task: ${displayTaskName} (ID: ${taskState.taskId})
...
Review the task context, status, and any previous notes or delegation messages to proceed.`;
      ...
      return { content: [{ type: "text", text: `Failed to continue task: ${err.message}` }], isError: true };
    }
  };
  server.tool(toolName, description, schemaRawShape, handler);
  registry.set(toolName, handler);
}
function registerTaskDashboardTool(server, registry) {
  ...
  const handler = async (args, extra) => {
    try {
      ...
      for (const dir of taskDirs) {
        if (dir.isDirectory()) {
          const individualTaskPath = await getTaskPath(dir.name);
      ...
      let markdown = "# \u{1F4CA} Task Dashboard\n\n| ID | Task Name | Status | Current Owner | Last Updated |\n|----|-----------|--------|---------------|--------------|\n";
      if (tasksData.length > 0) {
        tasksData.sort((a, b) => (a.id || "").localeCompare(b.id || ""));
...
`;
      ...
      return { content: [{ type: "text", text: `Failed to generate task dashboard: ${err.message}` }], isError: true };
    }
  };
  server.tool(toolName, description, schemaRawShape, handler);
  registry.set(toolName, handler);
}
function registerWorkflowMapTool(server, registry) {
  ...
  const handler = async (args, extra) => {
    ...
    const modes = [
      { id: "boomerang-mode", name: "\u{1FA83} Boomerang", description: "Task Intake, Analysis, Final Verification" },
      { id: "researcher-expert-mode", name: "\u{1F52C} Researcher", description: "In-depth Research (Optional)" },
      { id: "architect-mode", name: "\u{1F3DB}\uFE0F Architect", description: "Implementation Planning, Subtask Definition" },
      { id: "senior-developer-mode", name: "\u{1F468}\u200D\u{1F4BB} Senior Developer", description: "Subtask Implementation & Testing" },
      { id: "code-review-mode", name: "\u{1F50D} Code Review", description: "Quality Assurance, Verification" }
    ];
    let mapText = "# \u{1F5FA}\uFE0F Workflow Map\n\n";
    mapText += "```mermaid\ngraph TD\n";
    mapText += "    A[User Request] --> B(\u{1FA83} Boomerang Initial);\n";
    mapText += "    B -->|Needs Research?| C{Decision Point};\n";
    mapText += "    C -- Yes --> D(\u{1F52C} Researcher);\n";
    mapText += "    D --> B_Feeds_Research(\u{1FA83} Boomerang Integrates Research);\n";
    mapText += "    C -- No / Research Done --> E(\u{1F3DB}\uFE0F Architect);\n";
    mapText += "    B_Feeds_Research --> E;\n";
    mapText += "    E --> F(\u{1F468}\u200D\u{1F4BB} Senior Developer Subtasks);\n";
    mapText += "    F --> G{All Subtasks Done?};\n";
    mapText += "    G -- No --> F;\n";
    mapText += "    G -- Yes --> H(\u{1F3DB}\uFE0F Architect Verifies);\n";
    mapText += "    H --> I(\u{1F50D} Code Review);\n";
    mapText += "    I --> J{Review Approved?};\n";
    mapText += "    J -- No --> H;\n";
    mapText += "    J -- Yes --> K(\u{1FA83} Boomerang Final Verification);\n";
...
`;
    ...
    return { content: [{ type: "text", text: mapText }] };
  };
  server.tool(toolName, schemaRawShape, handler);
  registry.set(toolName, handler);
}
function registerTransitionRoleTool(server, registry) {
...
` : ""}The task is now with ${toEmoji} ${toRole.replace("-role", "")}. The new role should now take over.`;
      ...
      return { content: [{ type: "text", text: `Failed to transition role: ${err.message}` }], isError: true };
    }
  };
  server.tool(toolName, description, schemaRawShape, handler);
  registry.set(toolName, handler);
}
function registerWorkflowStatusTool(server, registry) {
  ...
  const handler = async (args, extra) => {
    ...
    try {
      ...
      const taskState = await getTaskState(taskPath);
      const displayTaskName = taskState.taskName || taskId;
      let workflow = `# \u{1F4DD} Workflow Status for: ${displayTaskName} (ID: ${taskState.taskId})
...
`;
      workflow += `**Progress**: ${taskState.progress !== void 0 ? (taskState.progress * 100).toFixed(0) + "%" : "N/A"}
`;
...
`;
      if (taskState.statusNotes && taskState.statusNotes.length > 0) {
        ...
        taskState.statusNotes.slice(-3).forEach((note) => {
...
`;
      ...
      if (taskState.notesLog && taskState.notesLog.length > 0) {
        ...
        taskState.notesLog.slice(-3).forEach((log) => {
...
`;
      ...
      return { content: [{ type: "text", text: `Failed to get workflow status: ${err.message}` }], isError: true };
    }
  };
  server.tool(toolName, description, schemaRawShape, handler);
  registry.set(toolName, handler);
...
// src/tools/createTaskTool.ts
function registerCreateTaskTool(server, registry) {
  ...
  const handler = async (args, extra) => {
    ...
    try {
      ...
      await fs2.mkdir(taskPath, { recursive: true });
      const initialState = {
        taskId,
        taskName,
      ...
      await fs2.writeFile(
        path2.join(taskPath, "task-state.json"),
        JSON.stringify(initialState, null, 2),
        "utf8"
      );
      if (taskDescFromArgs) {
        await fs2.writeFile(path2.join(taskPath, "task-description.md"), taskDescFromArgs, "utf8");
      }
      return {
        content: [
          {
            type: "text",
            text: `Task '${taskName}' (ID: ${taskId}) created successfully in folder '${path2.basename(taskPath)}'. Default owner: boomerang-mode.`
          }
        ]
      };
    } catch (err) {
...
// src/tools/getTaskContextTool.ts
import * as fs3 from "fs/promises";
import * as path3 from "path";
function registerGetTaskContextTool(server, registry) {
  ...
  const handler = async (args, extra) => {
    ...
    try {
      ...
      try {
        taskDescriptionContent = await fs3.readFile(
          path3.join(effectiveTaskPath, "task-description.md"),
          "utf8"
        );
      } catch (descErr) {
      ...
      try {
        implementationPlanContent = await fs3.readFile(
          path3.join(effectiveTaskPath, "implementation-plan.md"),
          "utf8"
        );
      } catch (planErr) {
        if (extra && extra.logger && typeof extra.logger.warn === "function") {
      ...
      const displayFolderName = path3.basename(effectiveTaskPath);
      const fullContext = {
        ...taskState,
      ...
      return {
        content: [
          { type: "text", text: `Failed to get task context for '${taskId}': ${err.message}` }
        ],
        isError: true
      };
    }
...
function registerUpdateTaskStatusTool(server, registry) {
  ...
  const handler = async (args, extra) => {
    ...
    try {
      ...
      if (progress !== void 0) taskState.progress = progress;
      if (notes) {
      ...
      taskState.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
      if (!taskState.createdAt) taskState.createdAt = (/* @__PURE__ */ new Date()).toISOString();
      if (taskName && !taskState.taskName) taskState.taskName = taskName;
      if (!taskState.taskId) taskState.taskId = taskId.split("-")[0];
      const statePath = path4.join(effectiveTaskPath, "task-state.json");
      await fs4.writeFile(statePath, JSON.stringify(taskState, null, 2), "utf8");
      const displayIdentifier = taskState.taskName || path4.basename(effectiveTaskPath);
      return {
        content: [
          {
            type: "text",
...
function registerListTasksTool(server, registry) {
  const toolName = "list_tasks";
  const description = "Lists all tasks (directories) in the task-tracking folder.";
  const schemaRawShape = {};
  const handler = async (args, extra) => {
    try {
      const folders = await fs5.readdir(TASK_TRACKING_PATH, { withFileTypes: true });
      ...
      return {
        content: [
          { type: "text", text: `Failed to list tasks: ${err.message}` }
        ],
        isError: true
      };
    }
...
function registerDeleteTaskTool(server, registry) {
  ...
  const handler = async (args, extra) => {
    ...
    try {
      ...
      await fs6.rm(effectiveTaskPath, { recursive: true, force: true });
      return {
        content: [
          {
            type: "text",
            text: `Task associated with ID/path '${taskId}' (resolved to ${path5.basename(effectiveTaskPath)}) deleted successfully.`
          }
        ]
      };
    } catch (err) {
...
// src/tools/addTaskNoteTool.ts
import * as fs7 from "fs/promises";
import * as path6 from "path";
function registerAddTaskNoteTool(server, registry) {
  ...
  const handler = async (args, extra) => {
    ...
    try {
      ...
      if (taskName && !taskState.taskName) taskState.taskName = taskName;
      if (!taskState.taskId) taskState.taskId = taskId.split("-")[0];
      const statePath = path6.join(effectiveTaskPath, "task-state.json");
      await fs7.writeFile(statePath, JSON.stringify(taskState, null, 2), "utf8");
      const displayIdentifier = taskState.taskName || path6.basename(effectiveTaskPath);
      return {
        content: [
          {
            type: "text",
...
// src/tools/updateTaskDescriptionTool.ts
import * as fs8 from "fs/promises";
import * as path7 from "path";
function registerUpdateTaskDescriptionTool(server, registry) {
  ...
  const schemaRawShape = {
    ...
    // Renamed from newDescription for consistency
  };
  const handler = async (args, extra) => {
    ...
    try {
      ...
      if (taskName && !taskState.taskName) taskState.taskName = taskName;
      if (!taskState.taskId) taskState.taskId = taskId.split("-")[0];
      const descriptionFilePath = path7.join(effectiveTaskPath, "task-description.md");
      await fs8.writeFile(descriptionFilePath, description, "utf8");
      const statePath = path7.join(effectiveTaskPath, "task-state.json");
      await fs8.writeFile(statePath, JSON.stringify(taskState, null, 2), "utf8");
      const displayIdentifier = taskState.taskName || path7.basename(effectiveTaskPath);
      return {
        content: [
          {
            type: "text",
...
// src/tools/getTaskStatusTool.ts
import * as path8 from "path";
function registerGetTaskStatusTool(server, registry) {
  ...
  const handler = async (args, extra) => {
    ...
    try {
      const effectiveTaskPath = await getTaskPath(taskId, taskName);
      const taskState = await getTaskState(effectiveTaskPath);
      const displayFolderName = path8.basename(effectiveTaskPath);
      return {
        content: [
          {
            type: "text",
            text: JSON.stringify({
              ...
              statusNotes: taskState.statusNotes ? taskState.statusNotes.slice(-1) : []
              // Last status note
            }, null, 2)
          }
        ]
...
// src/workflow-mcp-server.ts
import fs9 from "fs/promises";
import path9 from "path";
var toolHandlerRegistry = /* @__PURE__ */ new Map();
async function logToFile(message) {
  await fs9.appendFile("mcp-server-log.txt", `${(/* @__PURE__ */ new Date()).toISOString()}: ${message}
`);
}
function getServer() {
  ...
  const processCommandHandler = async ({ command_string }, extra) => {
    ...
    try {
      ...
      switch (command) {
        ...
        case "workflow-status":
          ...
          return await executeWorkflowStatusCommandLogic(cmdArgs);
        // Fallback or if no args
        case "research":
...
async function executeNextRoleCommandLogic(args) {
...
async function executeRoleCommandLogic(args) {
  if (args.length < 2) {
    return {
      content: [{
        type: "text",
  ...
  try {
    ...
    switch (roleName.toLowerCase()) {
      case "boomerang":
        roleId = "boomerang-role";
        break;
      case "researcher":
        roleId = "researcher-role";
        break;
      case "architect":
        roleId = "architect-role";
        break;
      case "developer":
      case "senior-developer":
        roleId = "senior-developer-role";
        break;
      case "review":
      case "code-review":
        roleId = "code-review-role";
        break;
      default:
        return {
          content: [{
...
async function executeWorkflowStatusCommandLogic(args) {
  const taskId = args[0] || "current";
  try {
...
async function executeResearchCommandLogic(args) {
  if (args.length < 2) {
    return {
      content: [{
        type: "text",
  ...
  try {
    ...
    const statePath = path9.join(taskPath, "task-state.json");
    await fs9.writeFile(statePath, JSON.stringify(taskState, null, 2), "utf8");
